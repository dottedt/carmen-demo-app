...
$ ->
  $('select#order_country_code').change (event) ->
    ...
    $('#order_city_wrapper').load("/orders/city_options")

  # If you also have cities, add this:
  $('#order_state_code_wrapper').change (event) ->
    city_wrapper = $('#order_city_wrapper')
    $('select', city_wrapper).attr('disabled', true)
    country_code = $('select#order_country_code').val()
    subregion_code = $('select#order_state_code').val()
    url = "/orders/city_options?country_code=#{country_code}&subregion_code=#{subregion_code}"
